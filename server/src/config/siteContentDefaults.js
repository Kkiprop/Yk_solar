...
export const cloneDefaultSiteContent = () => JSON.parse(JSON.stringify(defaultSiteContent));
  ...
  if (Array.isArray(defaults)) {
    ...
    return Array.from({ length: Math.max(defaults.length, overrides.length) }, (_, index) => {
      ...
      if (defaultItem === undefined) {
        return JSON.parse(JSON.stringify(overrideItem));
      }

      if (isObject(defaultItem) && isObject(overrideItem)) {
        return mergeSiteContent(defaultItem, overrideItem);
      }
